...
export default function EditQuizPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (isAdmin && quizId) {
      Promise.all([
        fetch(`/api/admin/quizzes/${quizId}`).then(res => res.json()),
        fetch(`/api/admin/quizzes/${quizId}/questions`).then(res => res.json())
      ])
      .then(([quizJson, questionsJson]) => {
        if (quizJson.data) {
          setMeta({
            title: quizJson.data.title || '',
            subject: quizJson.data.subject || '',
            description: quizJson.data.description || '',
            difficulty: quizJson.data.difficulty || 'medium',
            time_limit: quizJson.data.time_limit || 30,
            is_public: quizJson.data.is_public || false
          });
        }
        if (questionsJson.data) {
          setQuestions(questionsJson.data.map((q: any) => ({
            id: q.id,
          ...
          setQuestions([{ question: '', question_type: 'multiple_choice', options: ['', '', '', ''], correct_answer: '0', explanation: '', points: 1 }]);
        }
      })
      .catch(console.error)
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      // 1. Update Quiz Meta
      const qRes = await fetch(`/api/admin/quizzes/${quizId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(meta)
      });
      const qJson = await qRes.json();
      if (!qRes.ok) throw new Error(qJson.error || 'Failed to update quiz meta');

      // 2. Save Questions (Replaces all)
      ...
      if (!qsRes.ok) throw new Error(qsJson.error || 'Failed to update questions');
