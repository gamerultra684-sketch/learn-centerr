...
export default function CreateFlashcardPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      // 1. Create Deck
      const dRes = await fetch('/api/admin/flashcards', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(meta)
      });
      const dJson = await dRes.json();
      if (!dRes.ok) throw new Error(dJson.error || 'Failed to create deck');

      const deckId = dJson.data.id;
      ...
      if (!cRes.ok) throw new Error('Failed to save cards');
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold">Buat Deck Baru</h1>
      </div>
